...
export function CategoryForm({ category, onSubmit, onCancel, isLoading = false, error }: CategoryFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setValidationError("Kategori adı gereklidir");
      return;
    }
    ...
    try {
      ...
      setValidationError("Kategori kaydedilirken bir hata oluştu");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-2">
        ...
        >
          {isLoading ? "Kaydediliyor..." : category?.id ? "Güncelle" : "Oluştur"}
        </Button>
      </div>
    </form>
  );
